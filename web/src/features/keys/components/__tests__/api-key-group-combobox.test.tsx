...
describe('API key group combobox Auto effect', () => {
  ...
  test('selects another group from the compact quick switcher', async () => {
    const container = document.createElement('div')
    document.body.append(container)
    const root = createRoot(container)
    let selectedGroup = ''
    ...
    const trigger = getTrigger(container)
    assert.equal(trigger.getAttribute('aria-label'), 'Group')
    assert.equal(trigger.textContent?.includes('free'), true)

    await act(async () => trigger.click())
    const paidOption = getCommandItem('paid')
